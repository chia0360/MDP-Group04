...
int realignRideSide(){
  FindLeftRightSensorsAvg();
  if(rightSensorAvg<=4.7 )//&&rightSensorAvg>0 &&<=7 &&NewRightSensorAvg>0  )// && rightSensorAvg<=25  ||rightSensorAvg<14.0 &&rightSensorAvg>=0)//&&rightSensorAvg<=24 ||rightSensorAvg<14  )
  return 1;

  if(abs(rightSensorAvg-NewRightSensorAvg)>=4)
    return 1;
  else return 0;
}
...
void objScan(){
  ...
  if(rightSensorMedian<=13.0 && rightSensorMedian>0)
    objPos[4] = 1;
  else if(rightSensorMedian<=20 && rightSensorMedian>13.0)
    objPos[4] = 2;
  else
    objPos[4] = -2;
...
void RightReposition(){
   for(long startTime=millis();(millis()-startTime)<50;)
   FindLeftRightSensorsAvg() ;  
while(abs((NewRightSensorAvg+0.8)-rightSensorAvg )<1.0 ||(abs(NewRightSensorAvg +0.8)-rightSensorAvg)>3.5){
