...
double calibratedFrontCentreDist = 13.3 ;//9.6
...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    //  if ((commands >= 'a' && commands <= 'z') || (commands >='0' && commands <= '9')){ //if fastest path, send command
    //  break;
    if (commands != 'o') break;
    //
    if (commands == '\n') break;
  ...
  switch (commands) {
    case 'm':
      ...
      if (check == false) {
        ...
        if (!(rightSensorAvg >= 13.0 && rightSensorAvg <= 18.0 && NewRightSensorAvg >= 4.5 && NewRightSensorAvg <= 9.5) && rightSensorAvg <= 22 && rightSensorAvg <= 14) { //checkdistance>9.0 && checkdistance<=15 ||checkdistance<=8.7){
          //if((NewRightSensorAvg <= 7.00 && rightSensorAvg<=14.8) || (NewRightSensorAvg>=9.6 && rightSensorAvg>=18.2))
          //{
          //if(!((NewRightSensorAvg >= 7.01 && NewRightSensorAvg <=7.16 && rightSensorAvg>=16.1 &&rightSensorAvg<=16.7)))
          // {
          TurnAngle(-90);
...
void repositionRobotFront() {
  ...
  while ((abs(frontLeftSensorAvg - frontRightSensorAvg) > 0.95)) {
...
int realignRideSide() {
  ...
  if (rightSensorAvg >= 16.5 && rightSensorAvg <= 25  || rightSensorAvg < 14.0 && rightSensorAvg >= 0) //&&rightSensorAvg<=24 ||rightSensorAvg<14  )
  {
    //Serial.println("REPO");
    return 1;
  }
...
void objScan() {
  ...
  if (frontCentreSensorMedian <= 19 && frontCentreSensorMedian > 0)
    objPos[2] = 1;
  else if (frontCentreSensorMedian <= 28 && frontCentreSensorMedian > 19)
    objPos[2] = 2;
